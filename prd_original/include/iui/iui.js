...
//			if (window.iui_ext)	window.iui_ext.injectEventMethods(page);	// TG
	...
	showPageByHref: function(href, args, method, replace, cb)
	{
	  ...
	  function spbhCB(xhr) 
	  {
		if (xhr.readyState == 4)
		{
		  ...
		  {
              if( outsideHref.length > 1 )
                outsideHref = outsideHref[1];
              else
                outsideHref = null;
			  iui.insertPages(frag,outsideHref);
		  }
		  if (cb)
	...
	insertPages: function(frag, outsideTarget)
	{
...
addEventListener("click", function(event)
{
	...
	if (link)
	{
		...
		{
            linkHash = link.hash.match( /#[_]?(.*)/ )[1];
            if( !(hasId = $( linkHash )) && (link.href.split( "#" )[0] != window.location.href.split( "#" )) )
            {
	            iui.showPageByHref( link.href, null, "GET" );
            }
            else
            {
